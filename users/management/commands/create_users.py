...
class Command(BaseCommand):
    ...
    def add_users(self, number=20):
        ...
        for i in range(0,20):
            ...
            email = '%s.%s@%s' % (first_name_slug, last_name_slug, fake.free_email_domain())
    ...
    def add_questions(self, number=50):
        ...
        stack_res = requests.get("https://api.stackexchange.com/2.2/questions/featured?order=desc&sort=activity&site=stackoverflow").json()['items']
        for _ in range(0, number):
    ...
    def add_votes_question(self, number_max=30):
        ...
        print("Creating {:d} Votes on Questions".format(users.count() * question_count))

        for user in users:
            for _ in range(0, randint(1, number_max)):
                question = Question.objects.all()[randint(0, question_count-1)]
                if VoteQuestion.objects.filter(user=user, question=question).exists():
                    VoteQuestion.objects.get(user=user,question=question).delete()
                up = randint(0,1)==1
                VoteQuestion.objects.create(user=user, question=question, up=up)

    def add_votes_answer(self, number_max=30):
        users = User.objects.all()
        answer_count = Answer.objects.count()
        print("Creating {:d} Votes on Answers".format(users.count() * answer_count))
        
        for user in users:
            for _ in range(0, randint(1, number_max)):
                answer = Answer.objects.all()[randint(0, answer_count-1)]
                if VoteAnswer.objects.filter(user=user, answer=answer).exists():
                    VoteAnswer.objects.get(user=user, answer=answer).delete()
                VoteAnswer.objects.create(user=user, answer=answer, up=randint(0,1)==1)
